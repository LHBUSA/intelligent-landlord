    ...
    <header style={{
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 var(--page-pad)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60 }}>
        ...
        {isDesktop && (
          <nav style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            {NAV.map(n => (
              <Link key={n.href} href={n.href} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)', transition: 'color 0.2s' }}
      ...
      {isDesktop === false && open && (
        <div style={{ borderTop: '1px solid var(--border2)', background: 'var(--bg2)', padding: '12px var(--page-pad) 20px' }}>
          {NAV.map(n => (
            <Link key={n.href} href={n.href} onClick={() => setOpen(false)} style={{ display: 'block', padding: '10px 0', fontFamily: "'JetBrains Mono', monospace", fontSize: 12, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)', borderBottom: '1px solid var(--border2)' }}>
              {n.label}
            </Link>
          ))}
        </div>
